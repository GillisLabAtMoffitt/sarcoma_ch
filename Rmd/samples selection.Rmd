...
blood_patients <- blood_patients %>%
  ...
  mutate(blood_af_topoII = case_when(
    specimen_collection_date > treatment_start_date &
      is.na(blood_bf_topoII) &
      str_detect(treatment, topo_drugs)                                  ~ "Yes"
  )) %>%
  select(mrn, deidentified_patient_id, specimen_collection_date, 
         blood_bf_topoII, blood_af_topoII, treatment, everything()) %>% 
  ...
  mutate(days_from_topo_to_seqsample = case_when(
    blood_af_topoII == "Yes" |
  ...
  group_by(mrn) %>% 
  fill(blood_bf_topoII, days_from_presample_to_topo,
       # blood_af_chemo,
...
```{r}
samples <- blood_patients %>% 
  filter(blood_af_topoII == "Yes" | blood_during_topoII == "Yes") %>%
  select(deidentified_patient_id, sample_type, blood_bf_topoII, blood_during_topoII, 
         blood_af_topoII, days_from_presample_to_topo, days_from_topo_to_seqsample, treatment) %>% 
  # pivot_wider(id_cols = c(deidentified_patient_id),
  #             names_from = treatment_line, 
  #             values_from = treatment) %>% 
  group_by(deidentified_patient_id, sample_type, blood_bf_topoII, blood_af_topoII,
           blood_during_topoII, days_from_presample_to_topo, days_from_topo_to_seqsample) %>% 
...
write_csv(samples, "list of samples.csv")
```
