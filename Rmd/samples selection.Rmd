...
blood_patients <- blood_patients %>%
  ...
  # mutate(presample_id_topo = case_when(
  #   blood_bf_topoII == "Yes"                                             ~ sample_id
  # )) %>% 
  ...
  # mutate(presample_id_alk = case_when(
  #   blood_bf_alk == "Yes"                                               ~ sample_id
  # )) %>% 
  group_by(mrn) %>% 
  ...
  mutate(days_from_drugsend_to_after_sample = case_when(
    blood_af_topoII == "Yes" |
      blood_during_topoII == "Yes" |
      blood_af_chemo == "Yes" |
      blood_af_alk == "Yes"                                             ~ interval(
...
samples_topo <- blood_patients %>% 
  ...
  # select(mrn, deidentified_patient_id, date_of_diagnosis1,
  #        sample_type, blood_bf_topoII, blood_during_topoII, 
  #        blood_af_topoII, days_from_drugsend_to_after_sample, 
  #        days_from_drugstart_to_after_sample, treatment, 
  ...
  #          blood_during_topoII, days_from_drugsend_to_after_sample, 
  #          days_from_drugstart_to_after_sample, treatment_start_date, 
  #          duration_of_treatment) %>% 
  ...
  select(# mrn, 
         ...
         days_from_drugstart_to_after_sample,
         treatment,
         # starts_with("regimen"), 
         # treatment_start_date, treatment_end_date, 
         duration_of_treatment) %>% 
...
# write_csv(samples_topo, "list of sarcoma patients with samples topo.csv")
```
...
```{r}
samples_alk1 <- blood_patients %>%
  filter(have_good_presample_alk == "Yes" & blood_af_alk == "Yes") %>%

  select(mrn, deidentified_patient_id, date_of_diagnosis1, sample_type, have_good_presample_alk, 
         presample_id_alk, presample_date_alk,
         blood_af_alk, postsample_id_alk, postsample_date_alk,
         days_from_presample_to_alk, days_from_drugsend_to_after_sample, treatment, 
         treatment_start_date, treatment_end_date, duration_of_treatment) %>% 
  distinct() %>% 
  group_by(mrn, deidentified_patient_id, date_of_diagnosis1, sample_type, have_good_presample_alk, 
           presample_id_alk, presample_date_alk,
           blood_af_alk, postsample_id_alk, postsample_date_alk,
  ...
  select(mrn, 
         ...
         date_of_diagnosis1,
         sample_type, 
         have_good_presample_alk,
         presample_id_alk, presample_date_alk,
         blood_af_alk, 
         postsample_id_alk, postsample_date_alk,
         days_from_presample_to_alk, days_from_drugsend_to_after_sample,
         starts_with("regimen"), 
         treatment_start_date, treatment_end_date, 
         duration_of_treatment)
...
# write_csv(samples_alk, "list of sarcoma patients with samples alkylating.csv")
```
